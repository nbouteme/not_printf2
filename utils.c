...
** fixme: void* est tout le temps utilisé, donc dans le cas ou un nombre flottant
** est nécessaire, ca pete.
 */
...
void fill_data(t_dlisthead *l, va_list ap)
{
	...
	ftext_lstiterup(l, (void*)&read_and_fill, &tdp); // second pass
	ftext_lstiter(l, (void*)&save_data); // save
	free(datas);
}
